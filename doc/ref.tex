...
\item By providing just one nullary clause with $\allnc$, $\tonc$ only
  ...
  variants \texttt{ExNc}\index{ExNc} and \texttt{AndNc}\index{AndNc} of
...
\texttt{exd}, \texttt{exl}, \texttt{exr}, \texttt{exnc} written $\exd$,
$\exL$, $\exR$, $\exnc$ and also their total versions \texttt{exdt},
\texttt{exlt}, \texttt{exrt}, \texttt{exnct}, conjunction
\texttt{andd}, \texttt{andr}, \texttt{andnc} written $\landd$,
...
\texttt{mk-exnc}\index{mk-exnc@\texttt{mk-exnc}},
...
\texttt{mk-exnct}\index{mk-exnct@\texttt{mk-exnct}},
\texttt{mk-andd}\index{mk-andd@\texttt{mk-andd}},
\texttt{mk-andr}\index{mk-andr@\texttt{mk-andr}},
\texttt{mk-andnc}\index{mk-andnc@\texttt{mk-andnc}},
...
\begin{align*}
  ...
  \\
  \mathtt{ExNcTotalElim}\index{ExNcTotalElim@\texttt{ExNcTotalElim}} \colon
...
\index{ExRTotalIntro@\texttt{ExRTotalIntro}} and $\mathtt{ExNcTotalIntro}$%
\index{ExNcTotalIntro@\texttt{ExNcTotalIntro}}.  For the primitive
...
\texttt{simp} is defined via \texttt{simp-with}.  Also, for the
instantiated premises one can indicate how they are to come about.  So
in \texttt{(simp-with \textsl{opt-dir} \textsl{x}
  .\ \textsl{x-list})}\index{simp-with@\texttt{simp-with}},
\textsl{opt-dir} and \textsl{x} are as in \texttt{simp} (except that
the formula of $x$ must be an atom, a negated atom or $t \approx s$),
and \textsl{x-list} is a list consisting of
\begin{enumeratei}
\item a number or string identifying a hypothesis form the context,
\item the name of a theorem or global assumption,
...
\begin{alignat*}{2}
  ...
  \mu_n g<n \to g(\mu_n g),
  \\
  &\texttt{PropNatLeast}\colon &\quad&
  m \le n \to g m \to g(\mu_n g).
\end{alignat*}
